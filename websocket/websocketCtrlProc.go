...
import (
	...
	"github.com/use-go/websocket-streamserver/rtspcli"
	"github.com/use-go/websocket-streamserver/utils"
	"github.com/use-go/websocket-streamserver/wssapi"
)

type channelLink struct {
	rtspSocketChannel *rtspcli.SocketChannel
...
var (
	...
	channelSession map[string]*rtspcli.SocketChannel
	channelList    map[*websocket.Conn]*rtspcli.SocketChannel
)

func init() {
	//webSocketChannel = map[string]*websocket.Conn{}
	//rtspSocketChannel = map[string]*rtspcli.SocketChannel{}
	channelIndex = 0
	channelList = make(map[*websocket.Conn]*rtspcli.SocketChannel)
	channelSession = map[string]*rtspcli.SocketChannel{}
}
...
func (websockHandler *websocketHandler) initChannel(headers map[string]string) (strChannel string, err error) {
	...
	if _, exist := channelSession[strChannel]; !exist {
		...
		channleCli, err := rtspcli.Connect(rtspURI)
		if err != nil {
			return "", err
		}
